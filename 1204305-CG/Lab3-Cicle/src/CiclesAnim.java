...
public class CiclesAnim extends Window {
  private int circleStep = 0;
  private int ellipseStep = 0;
  private final int maxSteps = 360;

  public CiclesAnim() {
    super("Circles and Ellipses With Animation");
  }
  ...
  public void display(GLAutoDrawable drawable) {
    ...
    if (circleStep < maxSteps) {
      circleStep++;
    } else {
      circleStep = 0;
    }

    if (ellipseStep < maxSteps) {
      ellipseStep++;
    } else {
      ellipseStep = 0;
    }
  ...
  private void drawMidpointCircle(GL2 gl, int centerX, int centerY, int radius, boolean usePoints) {
    ...
    int pointsToDraw = (int)((circleStep / (float)maxSteps) * points.size());
